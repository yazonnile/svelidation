<script>
  ...
  const {
    ...
    createForm: hiddenCreateForm
  } = getValidation(getConfig(1));

  let activeId = 'use';
</script>

<Slide id="use" bind:activeId>
...
<Slide id="hidden" bind:activeId>
  <Form createForm={hiddenCreateForm}>
    <Entry store={hiddenStore} input={hiddenInput} />
  </Form>
</Slide>
